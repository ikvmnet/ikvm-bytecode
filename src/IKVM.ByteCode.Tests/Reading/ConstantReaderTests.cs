...
namespace IKVM.ByteCode.Tests.Reading
{
    ...
    public class ConstantReaderTests
    {
        ...
        {
            var c = ReadClass();
            c.Constants.Where(i => i.Kind == ConstantKind.Integer).Select(i => c.Constants.GetInteger((IntegerConstantHandle)i)).Should().Contain(i => i.Value == 394892);
        }
        ...
        {
            var c = ReadClass();
            c.Constants.Where(i => i.Kind == ConstantKind.Long).Select(i => c.Constants.GetLong((LongConstantHandle)i)).Should().Contain(i => i.Value == 34182132);
        }
        ...
        {
            var c = ReadClass();
            c.Constants.Where(i => i.Kind == ConstantKind.Float).Select(i => c.Constants.GetFloat((FloatConstantHandle)i)).Should().Contain(i => i.Value == 221.03f);
        }
